    ...
    public function count()
    {
        if($this->_rowCount >= 0) {
            ...
            if (null === $this->_countQb) {
                $em = $this->_qb->getEntityManager();
                $this->_countQb = clone $this->_qb;
                $from = $this->_qb->getDqlPart('from');
                $identifierNames = $em->getClassMetadata($from[0]->getFrom())->getIdentifierFieldNames();
                $alias = $this->_qb->getRootAlias();
                
                $this->_countQb->select('count(' . $alias . '.' . $identifierNames[0] . ') AS pag_count')
                               ->orderBy('pag_count');
            }
